...
#include "common/charconv_tables.hpp"
...
namespace behl
{
    ...
    {
        inline constexpr int kMaxDigits = 800;
        ...
        inline constexpr int kRyuIntervalShift = 2;

        inline constexpr std::array<int, 9> kPowTab{
            1,
            3,
            6,
            9,
        ...
        inline constexpr int kPowTabSize = static_cast<int>(kPowTab.size());
        inline constexpr int kMaxShift = 27;

        constexpr void trim(Decimal& a) noexcept
        {
        ...
        constexpr int decimal_length(uint64_t v) noexcept;
        constexpr void write_digits(uint64_t v, int nd, char* out) noexcept;

        constexpr bool prefix_less(const Decimal& a, uint64_t cut) noexcept
        {
            char buf[24];
        ...
        constexpr void right_shift(Decimal& a, int k) noexcept
        {
            int r = 0;
            int w = 0;
            uint64_t n = 0;
        ...
        constexpr void left_shift(Decimal& a, int k) noexcept
        {
        ...
        constexpr void shift(Decimal& a, int k) noexcept
        {
        ...
        constexpr bool should_round_up(const Decimal& a, int nd) noexcept
        {
        ...
        constexpr uint64_t rounded_integer(const Decimal& a) noexcept
        {
        ...
        template<typename T>
        constexpr uint64_t decimal_to_bits(Decimal& a, bool& overflow, bool& underflow) noexcept
        {
        ...
        inline constexpr std::array<double, 23> kPow10{
            1e0,
            1e1,
            1e2,
            1e3,
        ...
        inline constexpr int kFastPathMaxExp = 22;

        constexpr bool lower_eq(const char* p, const char* last, const char* lit, size_t n) noexcept
        {
        ...
        constexpr Scanned scan(const char* first, const char* last) noexcept
        {
            Scanned out;
            const char* p = first;
        ...
        constexpr uint64_t umul128(uint64_t a, uint64_t b, uint64_t& hi) noexcept;
        ...
        inline constexpr int kElDeclined = -1;

        constexpr int log2_pow10(int q) noexcept
        {
            return (((152170 + 65536) * q) >> 16) + 63;
        }

        constexpr EiselLemire eisel_lemire(uint64_t w, int q) noexcept
        {
        ...
        constexpr Decimal build_decimal(const char* first, const char* last) noexcept
        {
            Decimal d;
            const char* p = first;
            if (p != last && *p == '-')
        ...
        constexpr uint64_t umul128_portable(uint64_t a, uint64_t b, uint64_t& hi) noexcept
        {
        ...
        constexpr uint64_t umul128(uint64_t a, uint64_t b, uint64_t& hi) noexcept
        {
            ...
            {
                return umul128_portable(a, b, hi);
            }
        ...
        inline constexpr uint64_t kInverseOfFive = 14757395258967641293ull;
        inline constexpr uint64_t kMaxDivisibleByFive = std::numeric_limits<uint64_t>::max() / 5;

        constexpr uint32_t pow5_factor(uint64_t value) noexcept
        {
            uint32_t count = 0;
            for (;;)
            {
        ...
        constexpr bool multiple_of_pow5(uint64_t value, uint32_t p) noexcept
        {
            return pow5_factor(value) >= p;
        }

        constexpr bool multiple_of_pow2(uint64_t value, uint32_t p) noexcept
        {
            return (value & ((1ull << p) - 1)) == 0;
        }

        constexpr uint64_t mul_shift_64(uint64_t m, const uint64_t* mul, int j) noexcept
        {
            uint64_t high1 = 0;
        ...
        inline constexpr std::array<uint64_t, 20> kPow10U64{
            1ull,
            10ull,
            100ull,
            1000ull,
        ...
        inline constexpr char kDigits2[201] = "00010203040506070809"
        ...
        constexpr void write_two(char* out, uint32_t value) noexcept
        {
        ...
        constexpr RyuResult ryu_shortest(uint64_t ieee_mantissa, uint32_t ieee_exponent) noexcept
        {
            using Info = FloatInfo<double>;
        ...
        constexpr Decimal exact_decimal(uint64_t mant, int exp2, bool neg) noexcept
        {
            Decimal out;
            uint8_t tmp[24];
            int n = 0;
        ...
        constexpr int round_digits(const Decimal& src, int p, uint8_t* out, int& dp_out) noexcept
        {
            dp_out = src.decimal_point;

            if (p >= src.num_digits)
            {
                std::copy_n(src.digits, src.num_digits, out);
                return src.num_digits;
            }

            std::copy_n(src.digits, p, out);
            int nd = p;
        ...
        template<typename T>
        constexpr uint64_t bits_of_digits(const uint8_t* digits, int nd, int dp, bool neg) noexcept
        {
            Decimal t;
            std::copy_n(digits, nd, t.digits);
            t.num_digits = nd;
            t.decimal_point = dp;
            t.negative = neg;
        ...
        constexpr uint64_t digits_to_value(const uint8_t* digits, int nd) noexcept
        {
            uint64_t v = 0;
            for (int i = 0; i < nd; ++i)
            {
        ...
        template<typename T>
        constexpr Shortest shortest_digits(const Decimal& exact, uint64_t target, bool neg) noexcept
        {
            uint8_t digits[24];
        ...
        template<size_t N>
        constexpr to_chars_result emit(char* first, char* last, const char (&text)[N]) noexcept
        {
            ...
            std::copy_n(text, n, first);
            return { first + n, std::errc{} };
        }

        constexpr char* fill_small(char* dst, char c, int n) noexcept
        {
            for (int i = 0; i < n; ++i)
            {
                dst[i] = c;
        ...
        constexpr int rendered_length(const Shortest& s, bool neg) noexcept
        {
        ...
        constexpr to_chars_result render(char* first, char* last, const Shortest& s, bool neg) noexcept
        {
        ...
        template<typename T>
        constexpr to_chars_result to_chars_impl(char* first, char* last, T value) noexcept
        {
            using Info = FloatInfo<T>;
        ...
        template<typename T>
        constexpr from_chars_result from_chars_impl(const char* first, const char* last, T& value) noexcept
        {
        ...
        template<typename T>
        constexpr to_chars_result integer_to_chars(char* first, char* last, T value, int base) noexcept
        {
            bool negative = false;
            ...
            if constexpr (std::is_signed_v<T>)
            {
                ...
                {
                    negative = true;
                    magnitude = ~static_cast<uint64_t>(static_cast<int64_t>(value)) + uint64_t{ 1 };
                }
            }

            if (base == 10)
            {
                const int nd = decimal_length(magnitude);
                if (last - first < nd + (negative ? 1 : 0))
                {
                    return { last, std::errc::value_too_large };
                }
                if (negative)
                ...
                write_digits(magnitude, nd, first);
                return { first + nd, std::errc{} };
            }

            std::array<char, 64> buffer{};
            size_t digit_start = buffer.size();
            const auto radix = static_cast<uint64_t>(base);
            do
            {
                const auto digit = static_cast<int>(magnitude % radix);
                buffer[--digit_start] = static_cast<char>(digit < 10 ? '0' + digit : 'a' + digit - 10);
                magnitude /= radix;
            } while (magnitude != 0);

            const auto count = static_cast<ptrdiff_t>(buffer.size() - digit_start);
            if (last - first < count + (negative ? 1 : 0))
            {
                return { last, std::errc::value_too_large };
            }
            if (negative)
            ...
            std::copy_n(buffer.data() + digit_start, count, first);
            return { first + count, std::errc{} };
        }

        constexpr int digit_value(char c) noexcept
        {
            if (c >= '0' && c <= '9')
            {
                return c - '0';
        ...
        template<typename T>
        constexpr from_chars_result integer_from_chars(const char* first, const char* last, T& value, int base) noexcept
        {
            const char* p = first;
            bool negative = false;
            if constexpr (std::is_signed_v<T>)
            ...
            for (; p != last; ++p)
            {
                ...
                const auto d = static_cast<uint64_t>(digit);
                if (!overflow)
                {
                    if (magnitude > (std::numeric_limits<uint64_t>::max() - d) / radix)
                    {
                        overflow = true;
                    }
                    else
            ...
            uint64_t limit = static_cast<uint64_t>(std::numeric_limits<T>::max());
            if (negative)
            {
                limit += 1;
            }
            ...
            if (negative)
            {
                value = static_cast<T>(static_cast<int64_t>(~magnitude + uint64_t{ 1 }));
            }
            else
            {
    ...
    constexpr from_chars_result from_chars(const char* first, const char* last, double& value) noexcept
    {
        return charconv_detail::from_chars_impl(first, last, value);
    }

    constexpr from_chars_result from_chars(const char* first, const char* last, float& value) noexcept
    {
        return charconv_detail::from_chars_impl(first, last, value);
    }

    template<typename T>
    constexpr std::enable_if_t<std::is_integral_v<T> && !std::is_same_v<T, bool>, from_chars_result> from_chars(
    ...
    constexpr to_chars_result to_chars(char* first, char* last, double value) noexcept
    {
        return charconv_detail::to_chars_impl<double>(first, last, value);
    }

    constexpr to_chars_result to_chars(char* first, char* last, float value) noexcept
    {
        return charconv_detail::to_chars_impl<float>(first, last, value);
    }

    template<typename T>
    constexpr std::enable_if_t<std::is_integral_v<T> && !std::is_same_v<T, bool>, to_chars_result> to_chars(
