...
namespace behl
{
    ...
    static void reset_freereg(CompilerState& C, Reg result_reg)
    {
        C.freereg = static_cast<Reg>(result_reg + 1);
        if (C.freereg < C.min_freereg)
        {
    ...
    static void emit(CompilerState& C, Instruction instr, int32_t line = -1, int32_t column = -1)
    {
        ...
        {
            case OpCode::kOpJmp:
                C.current_proto->num_jump_targets += 1;
                break;
            case OpCode::kOpForPrep:
                C.current_proto->num_jump_targets += 3;
                break;
            case OpCode::kOpForLoop:
                C.current_proto->num_jump_targets += 2;
                break;
            default:
                break;
        }
    ...
    void VisitorAdapter::visit(const AstFuncDef& node)
    {
        ...
        if (!child.current_proto->code.empty())
        {
            ...
            ends_with_return = last_op == OpCode::kOpReturn || last_op == OpCode::kOpReturn0 || last_op == OpCode::kOpReturn1;
        }
        if (!ends_with_return)
        {
    ...
    void VisitorAdapter::visit(const AstReturn& node)
    {
        ...
        if (!has_defers && node.first_expr && !node.first_expr->next_child && node.first_expr->type == AstNodeType::kFuncCall)
        {
    ...
    static void emit_pending_defer_bodies(CompilerState& C, VisitorAdapter& visitor)
    {
        for (size_t i = 0; i < C.pending_defers.size(); ++i)
        {
            ...
            C.current_proto->defer_blocks[block] = DeferBlock{ static_cast<uint32_t>(C.current_proto->code.size()), link_reg };
