...
namespace behl
{
    ...
    constexpr size_t kEmitWordsPerOp = 3;
    ...
    {
        emit(0x9B000000u | (uint32_t{ rn(rhs) } << 16) | (uint32_t{ rn(addend) } << 10) | (uint32_t{ rn(lhs) } << 5) | rn(dst));
    }

    void A64Emitter::msub(A64Reg dst, A64Reg lhs, A64Reg rhs, A64Reg minuend)
    {
        emit(
            0x9B008000u | (uint32_t{ rn(rhs) } << 16) | (uint32_t{ rn(minuend) } << 10) | (uint32_t{ rn(lhs) } << 5) | rn(dst));
    }
    ...
    void A64Emitter::bind(A64Label label)
    {
        ...
        labels_[label.id] = LabelState{ static_cast<uint32_t>(buffer_.size()), static_cast<uint32_t>(nodes_.size()), true };
    }

    void A64Emitter::b(A64Label label)
    {
        nodes_.push_back(PatchNode{ static_cast<uint32_t>(buffer_.size()), label.id, 0, NodeType::kBranch, 0, 0 });
    }

    void A64Emitter::bcond(A64Cond cond, A64Label label)
    {
        nodes_.push_back(PatchNode{
            static_cast<uint32_t>(buffer_.size()), label.id, 0, NodeType::kBranchCond, static_cast<uint8_t>(cond), 0 });
    }

    void A64Emitter::reserve(size_t ops, size_t labels)
    {
        buffer_.reserve(ops * kEmitWordsPerOp);
        labels_.reserve(labels);
        nodes_.reserve(labels);
    }
    ...
    uint32_t A64Emitter::label_offset(uint32_t id) const noexcept
    {
        ...
        const uint32_t before = (state.node_index < nodes_.size())
            ...
            : (nodes_.empty() ? 0 : node_prefix_[nodes_.size() - 1] + nodes_.back().words);
    ...
    size_t A64Emitter::finalize(uintptr_t address, uint8_t* buf, size_t buf_size)
    {
        ...
        for (size_t i = 0; i < nodes_.size(); ++i)
        {
            ...
            switch (node.type)
            {
                ...
                {
                    const int64_t delta = (int64_t{ label_offset(node.label) } - int64_t{ own_offset });
                ...
                case NodeType::kCall:
                {
                    ...
                    else
                    {
                        ...
                        out[out_pos++] = 0xF2800000u | (1u << 21) | ((static_cast<uint32_t>(target >> 16) & 0xFFFFu) << 5)
                            | 16u;
                        out[out_pos++] = 0xF2800000u | (2u << 21) | ((static_cast<uint32_t>(target >> 32) & 0xFFFFu) << 5)
                            | 16u;
                        out[out_pos++] = 0xF2800000u | (3u << 21) | ((static_cast<uint32_t>(target >> 48) & 0xFFFFu) << 5)
                            | 16u;
