...
namespace behl
{
    ...
    constexpr size_t kCgOpsPerBytecode = 14;

    static size_t estimate_labels(uint32_t num_jump_targets) noexcept
    {
        return (static_cast<size_t>(num_jump_targets) * 13) / 10;
    }
    ...
    namespace
    {
        ...
        bool AbstractCompiler::compile()
        {
            ...
            out_.ops.reserve(n_ * kCgOpsPerBytecode);
            pc_labels_.reserve(n_);
            cold_blocks_.reserve(estimate_labels(proto_->num_jump_targets));
            for (size_t i = 0; i < n_; ++i)
            {
