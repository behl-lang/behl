...
namespace behl
{
    ...
    BEHL_API void debug_enable(State* S, bool enable = true);

    // Set event callback - called when VM hits breakpoint, completes step, etc.
    BEHL_API void debug_set_event_callback(State* S, DebugEventCallback callback);
    ...
    BEHL_API void debug_pause(State* S);

    // Query current debug state
    BEHL_API bool debug_is_enabled(State* S);
